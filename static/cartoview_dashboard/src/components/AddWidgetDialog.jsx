...
const AddWidgetDialog = ({ widgets, isOpen, onRequestClose, onWidgetSelect }) => {
  const widgetItems = Object.keys(Dashboard.widgetsClasses).map((key, index) => {
    return (
      <a href="#" key={index} className="list-group-item" onClick={() => onWidgetSelect(Dashboard.widgetsClasses[key])}>
  ...
  return (
    <Modal contentLabel={"Widget"} className="modal-dialog" isOpen={isOpen}>
